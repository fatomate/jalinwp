...
foreach (['core', 'tools', 'auth', 'approvals', 'oauth', 'server'] as $fg_oauth_module) {
    require_once '/wordpress/wp-content/plugins/fames-mcp-gateway/includes/class-' . $fg_oauth_module . '.php';
}
...
function fg_oauth_request(string $path, array $params = [], string $method = 'POST', array $headers = []): WP_REST_Request {
    $request = new WP_REST_Request($method, '/fames-mcp/v1' . $path);
...
function fg_oauth_wire(string $route, array $params, array $headers = [], string $method = 'POST'): array {
    ...
    $_SERVER['CONTENT_TYPE'] = $route === '/fames-mcp/v1/mcp' ? 'application/json' : 'application/x-www-form-urlencoded';
    unset($_SERVER['HTTP_AUTHORIZATION'], $_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'], $_SERVER['HTTP_ORIGIN']);
...
fg_oauth_case('public discovery advertises the exact MCP audience and S256 OAuth endpoints', static function(): void {
    ...
    fg_oauth_assert($metadata['issuer'] === untrailingslashit(rest_url('fames-mcp/v1/oauth/issuer.json')) && $metadata['issuer'] === FG_OAuth::issuer(), 'Issuer is not the stable REST-path identifier.');
    ...
    foreach (['token_endpoint' => 'token', 'registration_endpoint' => 'register', 'revocation_endpoint' => 'revoke'] as $field => $path) {
        fg_oauth_assert(($metadata[$field] ?? '') === rest_url('fames-mcp/v1/oauth/' . $path), 'Discovery endpoint does not match the configured WordPress REST URL.');
    }
...
fg_oauth_case('canonical discovery supports root, subdirectory, custom REST prefix, index permalinks and port installations', static function(): void {
    ...
    $cases = [
        ['https://gateway.example.test', 'wp-json', 'https://gateway.example.test', '/wp-json/fames-mcp/v1/oauth/issuer.json', '/%postname%/'],
        ['https://gateway.example.test/shop', 'wp-json', 'https://gateway.example.test', '/shop/wp-json/fames-mcp/v1/oauth/issuer.json', '/%postname%/'],
        ['https://gateway.example.test/shop', 'api', 'https://gateway.example.test', '/shop/api/fames-mcp/v1/oauth/issuer.json', '/%postname%/'],
        ['https://gateway.example.test:8443/shop', 'wp-json', 'https://gateway.example.test:8443', '/shop/wp-json/fames-mcp/v1/oauth/issuer.json', '/%postname%/'],
        ['https://gateway.example.test/shop', 'wp-json', 'https://gateway.example.test', '/shop/index.php/wp-json/fames-mcp/v1/oauth/issuer.json', '/index.php/%postname%/'],
    ];
    try {
...
fg_oauth_case('real WordPress REST serving accepts OAuth client_secret_basic despite native Application Password authentication', static function(): void {
    ...
    $wire = fg_oauth_wire('/fames-mcp/v1/oauth/token', $params, [
    ...
    $initialized = fg_oauth_wire('/fames-mcp/v1/mcp', ['jsonrpc' => '2.0', 'id' => 'oauth-wire', 'method' => 'initialize', 'params' => ['protocolVersion' => '2025-11-25', 'clientInfo' => (object) ['name' => 'synthetic-wire', 'version' => '1'], 'capabilities' => (object) []]], $headers);
    ...
    $notification = fg_oauth_wire('/fames-mcp/v1/mcp', ['jsonrpc' => '2.0', 'method' => 'notifications/initialized'], $headers);
    ...
    $fallback = fg_oauth_wire('/fames-mcp/v1/oauth/token', $fallback_params, ['PHP_AUTH_USER' => $client['client_id'], 'PHP_AUTH_PW' => $client['client_secret']]);
...
fg_oauth_case('native Application Password bypass follows the resolved OAuth route and ignores a conflicting query parameter', static function(): void {
    ...
    try {
        $_GET['rest_route'] = '/fames-mcp/v1/oauth/token';
        $_SERVER['REQUEST_URI'] = wp_parse_url(rest_url('fames-mcp/v1/oauth/token'), PHP_URL_PATH);
        ...
        $wp->query_vars['rest_route'] = '/fames-mcp/v1/oauth/token';
