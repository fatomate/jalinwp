...
foreach ( array( 'core', 'tools', 'wp-tools', 'design-blocks', 'page-layouts', 'page-design' ) as $module ) { require_once '/wordpress/wp-content/plugins/fames-mcp-gateway/includes/class-' . $module . '.php'; }
if ( ! defined( 'FG_DIR' ) ) { define( 'FG_DIR', '/wordpress/wp-content/plugins/fames-mcp-gateway/' ); }
if ( ! defined( 'FG_FILE' ) ) { define( 'FG_FILE', FG_DIR . 'fames-mcp-gateway.php' ); }
...
fg_design_case( 'classic canvas renders page content and required hooks without theme wrappers', static function(): void {
	...
	file_put_contents( $directory . '/style.css', "/*\nTheme Name: Fames Disposable Classic Fixture\nVersion: 1.0\n*/\n" );
	...
	$head = static function(): void { echo '<meta name="fames-hook-head" content="yes">'; };
	$body = static function(): void { echo '<span id="fames-hook-body"></span>'; };
	$footer = static function(): void { echo '<span id="fames-hook-footer"></span>'; };
	try {
		...
		foreach ( array( 'Canvas Content Marker', 'fames-hook-head', 'fames-hook-body', 'fames-hook-footer', '<main id="fames-canvas-main"' ) as $marker ) { fg_design_assert( str_contains( $rendered, $marker ), 'Missing canvas content/hook: ' . $marker ); }
