# JalinWP 0.3.3 — Admin And Connection Cleanup Update

This release simplifies the settings screens and adds explicit OAuth connection cleanup. It does not change MCP routes, OAuth discovery URLs, the plugin install path, saved page templates, or existing execution permissions.
...
1. Upload `jalinwp-0.3.3.zip` through WordPress Plugins → Add New → Upload Plugin, then replace the installed version.
2. Open Settings → JalinWP and confirm version `0.3.3` and the six tabs. Hard-refresh if cached admin styles remain.
3. Confirm the current access mode matches the previously saved choice. On a fresh installation, confirm Read Only before and after enabling MCP.
4. In OAuth Connections, clear old revoked/expired entries. Confirm an existing active client still works and its Activity Log history remains.
5. If testing complete reset, use Revoke And Clear All Connections, then reconnect a test client. The old credentials should no longer work.
6. Check the compact header, wrapping tabs, finance summary, keyboard focus, and long client/account labels on desktop and narrow screens.

The install directory remains `fames-mcp-gateway` so this is an in-place upgrade. There is no database schema increment in 0.3.3. Existing `FG_*` classes, `fg_*` storage, and `fames-mcp/v1` URLs remain intentional compatibility identifiers.
...
| `../test-runtime/admin-033-dom.cjs` | DOM checks for confirmation cancellation, confirmed submit, duplicate-submit prevention, and access-tab initialization. |

The full source archive contains the test wrappers, pinned runtime instructions, current evidence, sanitized screen fixtures, and `package-0.3.3.py`. See [Current Validation](VALIDATION.md) for executed tests and remaining staging checks.
