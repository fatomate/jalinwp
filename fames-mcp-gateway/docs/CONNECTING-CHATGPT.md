...
Version 0.3.1 adds opt-in YOLO Mode for writes without WordPress dashboard approval. Access Controls now offers Read Only, Reviewed Changes and YOLO Mode. It retains the one-click enable/disable, account setup, repaired connection list and client attribution from 0.3.0, together with the hosted OAuth callbacks, public discovery documents and optional connection trace introduced earlier. The plugin prepares public metadata when you open its settings or run Check Connection, provided its confirmed web root is writable. No Nginx configuration is edited. You do not need a WordPress Application Password, Node.js or the local bridge for this flow.
...
2. Upload **fames-mcp-gateway-0.3.1.zip**. If an earlier version is installed, choose the WordPress option to replace the existing plugin. Keep the same plugin directory, `fames-mcp-gateway`.
...
```text
https://firz.my/wp-json/fames-mcp/v1/mcp
```
...
| OAuth discovery fails | Run Check Connection after updating. The plugin prepares public metadata automatically where possible. If it reports a protected/unwritable directory or file conflict, retain the message. Existing files are preserved. Clear a stale discovery cache if applicable. |
...
```text
https://firz.my/wp-json/fames-mcp/v1/oauth/issuer.json
```

Its standard authorization-server discovery URL is:

```text
https://firz.my/.well-known/oauth-authorization-server/wp-json/fames-mcp/v1/oauth/issuer.json
```

The second document, needed by clients that discover OAuth before seeing an MCP challenge, is:

```text
https://firz.my/.well-known/oauth-protected-resource/wp-json/fames-mcp/v1/mcp
```
