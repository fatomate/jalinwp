...
define('FG_FILE', '/wordpress/wp-content/plugins/jalin-mcp-gateway/jalin-mcp-gateway.php');
foreach (['core', 'settings', 'oauth', 'discovery', 'connection-trace', 'admin'] as $module) { require_once '/wordpress/wp-content/plugins/jalin-mcp-gateway/includes/class-' . $module . '.php'; }
...
function trace_request(string $route = '/jalin-mcp/v1/oauth/register', string $method = 'POST'): WP_REST_Request {
...
update_option('fg_connection_trace_slot', '0', false);
$request = trace_request('/jalin-mcp/v1/mcp', 'GET'); $request->set_header('Authorization', 'Basic PRIVATE-SENTINEL');
