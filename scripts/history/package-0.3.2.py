"""Package the JalinWP branding update without dependencies or disposable sites."""
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "release-0.3.2")
    output = parser.parse_args().output.resolve()
    output.mkdir(parents=True, exist_ok=True)
    bootstrap = (PLUGIN / "jalin-mcp-gateway.php").read_text()
    ...
    sources.update(ROOT / name for name in [
        "START-HERE.md", "source-kit-members.json", "package-0.3.2.py",
        "test-runtime/brand-preview-tests.php",
    ])
    for name in ["test-runtime/evidence-0.3.2", "test-runtime/brand-preview"]:
    ...
    manifest = {
        ...
        "install_basename": "jalin-mcp-gateway/jalin-mcp-gateway.php",
    ...
    required = ["jalin-mcp-gateway.php", "assets/brand.css", "assets/admin.css",
    ...
    with zipfile.ZipFile(install) as a, zipfile.ZipFile(source) as b:
        ...
        for name in required:
            assert "jalin-mcp-gateway/" + name in a.namelist(), name
        assert len(set(a.namelist())) == len(a.namelist())
        assert len(set(b.namelist())) == len(b.namelist())
        for name in a.namelist():
            assert name.startswith("jalin-mcp-gateway/"), name
        ...
        report = {"status": "passed", "install_members": len(a.namelist()),
                  ...
                  "checks": ["ZIP integrity", "Legacy plugin basename", "Version agreement",
                             "Required brand assets", "Install/source byte identity", "Source hashes"]}
