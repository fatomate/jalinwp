# Fames MCP Gateway — Improvement and Implementation Plan
...
Canonical source reviewed: `/workspace/scratch/ee516bb02d35/fames-mcp-gateway`. Existing test runtime: `/workspace/scratch/ee516bb02d35/test-runtime`. Paths below are relative to the canonical plugin unless otherwise stated. Do not use the older `github/` workspace copy as the implementation baseline.
...
| 8. Page template/layout support | Choose Theme Default, a compatible existing template, or a plugin-owned **Fames Canvas** without theme header/footer/sidebar/page-title wrappers. | D |
...
| `fames-mcp-gateway.php`, `readme.txt`, `README.md` | Load modules/assets; update version only with a tested release; document capabilities and compatibility. |
