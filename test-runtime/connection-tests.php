...
require_once '/wordpress/wp-content/plugins/fames-mcp-gateway/includes/class-server.php';
require_once '/wordpress/wp-content/plugins/fames-mcp-gateway/includes/class-oauth.php';
require_once '/wordpress/wp-content/plugins/fames-mcp-gateway/includes/class-connection.php';
...
function fg_diag_metadata(): array {
    return [
        ...
        'token_endpoint'=>rest_url('fames-mcp/v1/oauth/token'),
        'registration_endpoint'=>rest_url('fames-mcp/v1/oauth/register'),
...
add_filter('pre_http_request', static function($pre,$args,$url) use (&$mode,&$requests) {
    ...
    if (in_array($url,FG_OAuth::authorization_metadata_urls(),true)) {
        ...
        if ($mode==='redirect') { return ['headers'=>['location'=>rest_url('fames-mcp/v1/oauth/authorization-server')],'body'=>'','response'=>['code'=>302,'message'=>'Fixture']]; }
    ...
    if ($url===rest_url('fames-mcp/v1/oauth/authorization-server')) { return fg_diag_response(200,fg_diag_metadata()); }
...
fg_diag_assert('Working raw REST metadata cannot hide a missing canonical discovery URL',fg_diag_status($report,'OAuth discovery')==='fail' && count($requests)===5 && !in_array(rest_url('fames-mcp/v1/oauth/authorization-server'),array_column($requests,'url'),true));
$raw=wp_safe_remote_get(rest_url('fames-mcp/v1/oauth/authorization-server'));
