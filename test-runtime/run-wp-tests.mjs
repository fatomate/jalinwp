...
const args = [path.join(root,'node_modules/@wp-playground/cli/wp-playground.js'),'run-blueprint',
 ...
 `--mount-dir=${path.resolve(root,'../fames-mcp-gateway')}`,'/wordpress/wp-content/plugins/fames-mcp-gateway',
...
try {
 ...
 try {
  ...
  if (Array.isArray(summary.cases) && summary.cases.some(c=>c.pass===false)) success=false;
...
catch(e){console.error(`No complete test result: ${e.message}`);}
