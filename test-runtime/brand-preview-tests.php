...
foreach (['woocommerce/woocommerce.php', 'fames-mcp-gateway/fames-mcp-gateway.php'] as $plugin) {
...
foreach ($variants as $name=>$tab) {
    ...
    update_option('fg_settings', $settings, false);
    $_GET = ['page'=>'fames-mcp-gateway', 'tab'=>$tab];
    $_SERVER['REQUEST_URI'] = '/wp-admin/options-general.php?page=fames-mcp-gateway&tab=' . $tab;
    ob_start();
    FG_Admin::page();
    ...
    jalin_preview_check($name . ': old display name absent', !str_contains($content, 'Fames MCP Gateway'));
    if ($name === 'finance') {
    ...
    $content = preg_replace_callback('/href="([^"]*)"/', static function ($match) {
        ...
        if (str_contains($href, 'options-general.php?page=fames-mcp-gateway')) {
