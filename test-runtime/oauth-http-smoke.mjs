...
const issuer = `${site}/wp-json/fames-mcp/v1/oauth/issuer.json`;
const resource = `${site}/wp-json/fames-mcp/v1/mcp`;
...
const staticMetadataTypes = new Map([
    ['/.well-known/oauth-authorization-server/wp-json/fames-mcp/v1/oauth/issuer.json', 'application/json'],
    ['/.well-known/oauth-protected-resource/wp-json/fames-mcp/v1/mcp', 'application/octet-stream'],
]);
...
$activation = activate_plugin('fames-mcp-gateway/fames-mcp-gateway.php');
...
`;
    ...
    server = spawn(process.execPath, [path.join(root, 'node_modules/@wp-playground/cli/wp-playground.js'), 'server',
        ...
        `--mount-dir=${path.resolve(root, '../fames-mcp-gateway')}`, '/wordpress/wp-content/plugins/fames-mcp-gateway', '--verbosity=quiet'],
        { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
    server.stdout.on('data', b => { output = (output + b.toString()).slice(-12000); });
    server.stderr.on('data', b => { output = (output + b.toString()).slice(-12000); });
        ...
        if ((mode === 'blocked-root' && incomingURL.pathname.startsWith('/.well-known/')) || (mode==='gridpane-static' && incomingURL.pathname.includes('/.well-known'))) {
            ...
            if (mode==='gridpane-static' && incoming.method==='GET' && contentType) {
                try {
                    ...
                    outgoing.end(data);
                    return;
                } catch(error) { if(error.code!=='ENOENT') { outgoing.writeHead(500); outgoing.end('Fixture static read failed'); return; } }
            }
            blockedRequests++;
    ...
    for (let attempt=0; attempt<100; attempt++) {
        ...
        try { const r=await request('/wp-json/fames-mcp/v1/oauth/authorization-server'); if(r.status===200){ ready=true; break; } } catch {}
    ...
    if (mode === 'blocked-root') {
        ...
        check('Proactive discovery uses the path-specific protected resource and RFC8414 issuer documents', proactiveDiscoveryAttempts.length===2 && proactiveDiscoveryAttempts[0].path==='/.well-known/oauth-protected-resource/wp-json/fames-mcp/v1/mcp' && proactiveDiscoveryAttempts[1].path==='/.well-known/oauth-authorization-server/wp-json/fames-mcp/v1/oauth/issuer.json' && proactiveDiscoveryAttempts.every(attempt=>attempt.status===200));
    ...
    const resourceURL = /resource_metadata="([^"]+)"/.exec(unauthenticated.headers.get('www-authenticate'))[1];
    ...
    const expectedDiscoveryPaths = [
        '/.well-known/oauth-authorization-server/wp-json/fames-mcp/v1/oauth/issuer.json',
        '/.well-known/openid-configuration/wp-json/fames-mcp/v1/oauth/issuer.json',
        '/wp-json/fames-mcp/v1/oauth/issuer.json/.well-known/openid-configuration',
    ];
    if (mode === 'blocked-root') {
        ...
        const reachableMetadata = await request('/wp-json/fames-mcp/v1/oauth/authorization-server');
    ...
    check('REST metadata responses remain uncached', (await request('/wp-json/fames-mcp/v1/oauth/authorization-server')).headers.get('cache-control')?.includes('no-store'));
    ...
    await request(loginURL,{browser:true});
    const login=await request('/wp-login.php',{browser:true,method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({log:'fg_oauth_http_admin',pwd:password,'wp-submit':'Log In',redirect_to:new URL(loginURL).searchParams.get('redirect_to')||auth.href,testcookie:'1'})});
    ...
    previewHtml = previewHtml.replace(/https:\/\/gateway\.example\/wp-content\/plugins\/fames-mcp-gateway\/assets\//g, 'assets/');
    await fs.writeFile(path.join(previewDir, `oauth-${writeMode}.html`), previewHtml);
    await fs.cp(path.resolve(root, '../fames-mcp-gateway/assets'), path.join(previewDir, 'assets'), {recursive: true});
    const logo = await request('/wp-content/plugins/fames-mcp-gateway/assets/brand/jalinwp-logo-horizontal-white.png');
    ...
    const approve=buttons.find(b=>/approve|allow/i.test(b[2]));
    check('Consent contains a named approval control',!!approve);
    ...
    if (writeMode === 'yolo') {
        check('Consented OAuth connection advertises immediate YOLO writes', /YOLO Mode is active/.test(initialized.result?.instructions || '') && listing.result?.tools.some(t=>t.name==='wp_content_create' && t.description.startsWith('YOLO Mode:')));
        ...
        check('MCP catalog respects a read-only consent grant',Array.isArray(listing.result?.tools) && listing.result.tools.some(t=>t.name==='wp_content_list') && !listing.result.tools.some(t=>t.name==='wp_content_create'));
    }
    ...
    const report={status:'failed',mode,clientType,writeMode,passed:cases.length,cases,error:error.message};
    await fs.writeFile(reportPath,JSON.stringify(report,null,2));
    console.error(JSON.stringify(report,null,2));
    process.exitCode=1;
} finally {
    if(proxy) await new Promise(resolve => { proxy.closeAllConnections(); proxy.close(resolve); });
