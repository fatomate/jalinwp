...
The endpoint is `https://your-site.example/wp-json/jalin-mcp/v1/mcp`; use the exact URL displayed by WordPress for installations in a subdirectory. OAuth tokens authorize this gateway only. The plugin does not turn them into general WordPress REST credentials or forward them to Stripe, Billplz or another service.
