...
Preserve the extracted repository layout: `jalin-mcp-gateway/` at the root and the disposable runtime at `scripts/test-runtime/`. The runtime contains WordPress setup, test cases, wrappers, and the runner; retained historical evidence is under `docs/evidence/`.

## Install the Pinned Runtimes

Requires Node.js 22+, npm, and internet access for initial downloads. From `scripts/test-runtime/`:
...
Do not add `--ignore-scripts`: on Node 26, `fs-ext-extra-prebuilt` needs its install build.
...
Run from `scripts/test-runtime/` after installing the pinned runtimes:
...
The brand fixture renders six actual tabs plus the disabled setup state into `brand-preview/`; exported forms cannot submit. These PHP/DOM results do not establish visual browser appearance. Current executed checks are recorded in [Validation](VALIDATION.md); repository-root `docs/evidence/evidence-0.3.3/` contains historical snapshots, not evidence for regenerated previews.

## Existing 0.3.1 Feature Suites

Run from `scripts/test-runtime/`, one named test at a time:
...
node cases/admin-ui-030.mjs ui-preview design-js-runtime setup-ui-dom-output.json
```
...
`admin-connection-tests.php` now covers the current bootstrap, section-scoped settings versions, real OAuth fixture rows, six-tab rendering, revoke failures, and admin action guards. `admin-033-tests.php` remains the broader current admin workflow suite. Earlier 0.3.0 results are prior evidence only unless a suite was explicitly rerun for 0.3.1. Neither the presence of an old result file nor its original pass status proves coverage of this build.
...
## Native MySQL and MariaDB Gate

See [Native Database Gate](NATIVE-030.md) for the isolated runner and prerequisites. Its directory name is retained; its current assertions expect OAuth schema **3**, including `allow_yolo`, while the core schema marker remains `0.3.0`. The initial 0.3.3 candidate passed **14/14** using PHP **8.3.33** with `mysqli` and MariaDB **11.4.13** in disposable Docker containers, with the generated database removed. See [current validation](VALIDATION.md). A separate MySQL run and native WooCommerce/HPOS remain untested; PHP-WASM checks are not native database evidence.
...
Repeat against a separate local MariaDB service and save `native-mariadb-030-result.json`. The runner only accepts a loopback service or an explicit local Unix socket; it generates, installs into, and removes a fresh database. Do not point a tunnel at a live database. A pass requires exit code zero, `status: "passed"`, all gate cases passing, and `disposable_database_removed: true`.
...
Every WordPress test copies the fixture to a disposable temporary site. Plugin source mounts at `/wordpress/wp-content/plugins/jalin-mcp-gateway`; this runtime mounts at `/test-results`. Temporary cleanup retries at most three times for a late Playground mount flush. Test mutations must remain synthetic and guarded by `FG_TEST_DISPOSABLE` where required.

The runner exits nonzero for assertion failure, exceptions, PHP fatal errors, incomplete or missing results, or a nonzero Playground exit. `<test-name>-output.json` records status and captured output. Run a named suite once at a time to avoid overwriting its result. Some suites also generate separate evidence or markup fixtures. Repository-root `docs/evidence/` contains historical snapshots, not a test runner or current validation.

```sh
node run-wp-tests.mjs runtime-smoke.php
node run-wp-tests.mjs core-runtime-smoke.php
node node_modules/@php-wasm/cli/php-wasm.js -l ../../jalin-mcp-gateway/jalin-mcp-gateway.php
node --check ../../jalin-mcp-gateway/assets/admin.js
node --check ../../jalin-mcp-gateway/assets/finance.js
node --check ../../jalin-mcp-gateway/assets/design-validation.js
node --test ../bridge/test.mjs
node node_modules/@php-wasm/cli/php-wasm.js cases/analytics.php
```

Lint every changed PHP file, not only the bootstrap. Historical `php-lint-0.3.1.json` and `js-syntax-0.3.1.json` under repository-root `docs/evidence/` record syntax-only results with source SHA-256 values; passing syntax does not execute the code or establish native database compatibility. Standalone PHP-WASM can report a different PHP version from the WordPress runner. Before production use, address the unverified environments in repository-root [docs/VALIDATION.md](VALIDATION.md), including the actual host, browser, editor, theme, database, WooCommerce configuration, and hosted clients.
