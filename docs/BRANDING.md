...
- The page layout label is **JalinWP Canvas**. Its stored identifier stays `jalin-mcp-canvas`.
...
The install ZIP intentionally contains `jalin-mcp-gateway/jalin-mcp-gateway.php`. Keeping the existing basename makes this an in-place update. Do not rename the installed directory or install a second copy alongside it.

## Compatibility Decisions

| Kept Identifier Or Behavior | Reason |
| --- | --- |
| `jalin-mcp-gateway/` and bootstrap filename | Existing WordPress activation identity and in-place upgrades |
| `jalin-mcp/v1` MCP/OAuth endpoints and issuer/resource URLs | Existing registrations, tokens, connector URLs, and metadata discovery |
...
| `jalin-mcp-canvas` and `jalin-mcp-gateway//jalin-mcp-canvas` | Saved templates and existing pages |
...
| `jalin-mcp-gateway.php`, `readme.txt` | Public plugin metadata and release version |
