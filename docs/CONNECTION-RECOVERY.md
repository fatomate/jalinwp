# Historical JalinWP 0.2.2 Connection Recovery
...
1. Upload `jalin-mcp-gateway-0.2.2.zip` through WordPress and replace the existing plugin.
2. Open **Settings → JalinWP** and confirm version **0.2.2**. Keep the gateway/OAuth enabled and your account allowed.
...
4. Use the same MCP URL displayed by the plugin. For the reported site it is `https://firz.my/wp-json/jalin-mcp/v1/mcp`.
...
| Field | Value |
| --- | --- |
| MCP server URL | `https://firz.my/wp-json/jalin-mcp/v1/mcp` |
| Issuer | `https://firz.my/wp-json/jalin-mcp/v1/oauth/issuer.json` |
| Authorization endpoint | `https://firz.my/wp-admin/admin-post.php?action=fg_oauth_authorize` |
| Token endpoint | `https://firz.my/wp-json/jalin-mcp/v1/oauth/token` |
| Registration endpoint | `https://firz.my/wp-json/jalin-mcp/v1/oauth/register` |
| Scope | `mcp` |
...
| --- | --- |
| OAuth authorization server | `/.well-known/oauth-authorization-server/wp-json/jalin-mcp/v1/oauth/issuer.json` |
| MCP protected resource | `/.well-known/oauth-protected-resource/wp-json/jalin-mcp/v1/mcp` |
...
| Module loading and release version | `jalin-mcp-gateway.php` |
| OAuth callback/consent regression cases | `tests/oauth.php` |
| Publisher safety tests | `scripts/test-runtime/discovery-publisher-tests.php` |
| Controlled discovery tests | `scripts/test-runtime/connection-tests.php` |
| Trace/admin/report safety tests | `scripts/test-runtime/connection-trace-tests.php` |
| Official SDK proactive discovery and full HTTP flow | `scripts/test-runtime/oauth-http-smoke.mjs` |

The source ZIP includes the complete plugin, pinned runtime dependencies, test scripts, and retained historical evidence. Follow [Developer Tests](DEVELOPER-TESTS.md) to reproduce on disposable fixtures. Do not run destructive fixtures against a real shop. See [Historical Validation](VALIDATION.md) for earlier results and [OAuth Development](OAUTH-DEVELOPMENT.md) for the full OAuth lifecycle and finance/tool change locations.
