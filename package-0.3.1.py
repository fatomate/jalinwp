...
plugin=base/'fames-mcp-gateway'; runtime=base/'test-runtime'; release=base/'release'
release.mkdir(exist_ok=True)
version='0.3.1'; prefix='fames-mcp-gateway-source/'
def sha(data): return hashlib.sha256(data).hexdigest()
source_data={}
for path in sorted(plugin.rglob('*')):
 if path.is_file() and not any(x.startswith('.') for x in path.relative_to(plugin).parts):
  source_data['fames-mcp-gateway/'+path.relative_to(plugin).as_posix()]=path.read_bytes()
install_data={n:d for n,d in source_data.items() if '/tests/' not in n and n!='fames-mcp-gateway/bridge/test.mjs'}
assert b'Version: 0.3.1' in install_data['fames-mcp-gateway/fames-mcp-gateway.php']
modules=re.search(r"foreach \(\[(.*?)\] as \$fg_module\)",install_data['fames-mcp-gateway/fames-mcp-gateway.php'].decode()).group(1)
for module in re.findall(r"'([^']+)'",modules): assert 'fames-mcp-gateway/includes/class-'+module+'.php' in install_data,module
for name in ['class-settings.php','class-page-design.php','class-design-blocks.php','class-finance-admin.php']:
 assert 'fames-mcp-gateway/includes/'+name in install_data
assert all('/tests/' not in n for n in install_data)
install=release/f'fames-mcp-gateway-{version}.zip'
...
guide=release/f'Fames-MCP-Gateway-YOLO-Handoff-v{version}.md'
source_data[guide.name]=guide.read_bytes()
source_data['Fames-MCP-Gateway-Developer-Handoff-v0.3.0.md']=(release/'Fames-MCP-Gateway-Developer-Handoff-v0.3.0.md').read_bytes()
source_data['Fames-MCP-Gateway-Improvement-Plan.md']=(release/'Fames-MCP-Gateway-Improvement-Plan.md').read_bytes()
...
source_data['START-HERE.md']=f'''# Fames MCP Gateway {version} Complete Source

Use `fames-mcp-gateway-{version}.zip` for staging installation. This source archive is the full development kit and is not an installable plugin ZIP.

Read `{guide.name}` for the current YOLO update, setup steps, response contract, source map, migration and actual tests. The included 0.3.0 developer handoff describes the previous nine improvements; it is historical when it conflicts with the current handoff. Current docs are under `fames-mcp-gateway/docs/`, including `YOLO-MODE.md`, `PAGE-DESIGN.md` and `VALIDATION.md`.

Keep `fames-mcp-gateway/` and `test-runtime/` side by side. Follow `test-runtime/DEVELOPER-TESTS.md` for pinned dependencies and disposable fixtures. Evidence under `test-runtime/evidence/` contains only the checks indexed for this 0.3.1 build; older tests remain in the source but were not all rerun. Native MySQL/MariaDB, actual clients/staging, browser/editor and theme/version matrix gates remain pending.
...
'''.encode()
manifest={'plugin':'Fames MCP Gateway','version':version,'date':'2026-09-05','install_zip':install.name,'install_zip_sha256':sha(install.read_bytes()),'files':{n:sha(d) for n,d in sorted(source_data.items())},'pending_gates':index['not_executed']}
source_data['SOURCE-MANIFEST.json']=(json.dumps(manifest,indent=2)+'\n').encode()
source=release/f'fames-mcp-gateway-{version}-source.zip'
