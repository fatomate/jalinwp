(() => {
    ...
    document.querySelectorAll('.fg-finance-form').forEach((form) => {
        ...
        if (!(button instanceof HTMLButtonElement) || !(output instanceof HTMLElement) || !(order instanceof HTMLInputElement)) return;
        let generation = 0;
        ...
        const finance = window.fgFinance;
        if (finance) button.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(finance.ajaxUrl, {method: 'POST', body: data, credentials: 'same-origin', signal: requestController.signal});
                /** @type {FGFinanceResponse} */
                const result = await response.json();
                if (run !== generation) return;
                if (!response.ok || !result.success || !result.data || !('rows' in result.data)) throw new Error(result.data?.message || 'The order could not be tested. Reload the page and retry.');
                ...
                if (run === generation) output.replaceChildren(element('p', error instanceof Error && error.name === 'AbortError' ? 'The order test timed out. Retry with the same Order ID.' : error instanceof Error ? error.message : 'The order could not be tested. Reload the page and retry.', 'fg-finance-notice'));
            } finally {
        ...
        form.addEventListener('submit', () => {
            ...
            if (save instanceof HTMLButtonElement || save instanceof HTMLInputElement) { save.disabled = true; save.textContent = 'Saving…'; }
        });
    });
})();
