...
final class FG_OAuth {
    ...
    private static function is_client_endpoint(): bool {
        global $wp;
        $allowed = ['/jalin-mcp/v1/oauth/token', '/jalin-mcp/v1/oauth/revoke'];
        ...
        foreach (['token', 'revoke'] as $endpoint) {
            $url = rest_url('jalin-mcp/v1/oauth/' . $endpoint);
    ...
    public static function register(): void {
        foreach (['register' => 'register_client', 'token' => 'token', 'revoke' => 'revoke_token'] as $path => $callback) {
            register_rest_route('jalin-mcp/v1', '/oauth/' . $path, ['methods' => 'POST', 'callback' => [self::class, $callback], 'permission_callback' => '__return_true']);
        }
        foreach (['authorization-server' => 'authorization_metadata', 'protected-resource' => 'resource_metadata'] as $path => $callback) {
            register_rest_route('jalin-mcp/v1', '/oauth/' . $path, ['methods' => 'GET',
                'callback' => static function () use ($callback) { return self::response(self::$callback()); }, 'permission_callback' => '__return_true']);
        }
    }
    public static function resource(): string { return rest_url('jalin-mcp/v1/mcp'); }
    ...
    public static function issuer(): string { return untrailingslashit(rest_url('jalin-mcp/v1/oauth/issuer.json')); }
    public static function resource_metadata_url(): string { return rest_url('jalin-mcp/v1/oauth/protected-resource'); }
    ...
    public static function authorization_metadata(): array {
        return ['issuer' => self::issuer(),
            ...
            'token_endpoint' => rest_url('jalin-mcp/v1/oauth/token'), 'registration_endpoint' => rest_url('jalin-mcp/v1/oauth/register'),
            'revocation_endpoint' => rest_url('jalin-mcp/v1/oauth/revoke'), 'response_types_supported' => ['code'],
    ...
    private static function secure_urls(): bool {
        ...
        foreach ([self::issuer(), self::resource(), admin_url('admin-post.php'), rest_url('jalin-mcp/v1/oauth/token')] as $url) {
