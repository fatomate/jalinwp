...
final class FG_Admin {
    public static function boot(): void {
        add_action('admin_menu', static function () {
            add_options_page('JalinWP', 'JalinWP', 'manage_options', 'jalin-mcp-gateway', [self::class, 'page']);
        });
        ...
        add_action('admin_enqueue_scripts', static function ($hook) {
            if ($hook === 'settings_page_jalin-mcp-gateway') {
                wp_enqueue_style('jalinwp-brand', plugins_url('assets/brand.css', FG_FILE), [], FG_VERSION);
                wp_enqueue_style('jalin-mcp', plugins_url('assets/admin.css', FG_FILE), ['jalinwp-brand'], FG_VERSION);
                wp_enqueue_script('jalin-mcp-admin', plugins_url('assets/admin.js', FG_FILE), [], FG_VERSION, true);
                wp_localize_script('jalin-mcp-admin', 'fgConnection', ['ajaxUrl' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('fg_connection_check')]);
            }
        });
    }
    ...
    private static function finish_settings($result, array $input = [], string $tab = 'access'): void {
        $url = admin_url('options-general.php?page=jalin-mcp-gateway&tab=' . ($tab === 'connect' ? 'connect' : 'access'));
    ...
    public static function quick_connect(): void {
        ...
        $result = FG_Settings::enable_for_user(get_current_user_id(), self::expected());
        if (!is_wp_error($result) && class_exists('FG_Discovery')) { FG_Discovery::publish(); }
        self::finish_settings($result, [], 'connect');
    }
    ...
    public static function connection_check(): void {
        ...
        wp_safe_redirect(admin_url('options-general.php?page=jalin-mcp-gateway&tab=connect&checked=1#fg-checks'));
        exit;
    }
    ...
    public static function start_connection_trace(): void {
        ...
        wp_safe_redirect(admin_url('options-general.php?page=jalin-mcp-gateway&tab=connect&trace_started=1#fg-trace'));
        exit;
    }
    ...
    public static function revoke_connection(): void {
        ...
        wp_safe_redirect(admin_url('options-general.php?page=jalin-mcp-gateway&tab=connections&revoked=1#fg-connections'));
        exit;
    }
    private static function finish_connection_cleanup(array|WP_Error $result, string $outcome): void {
        ...
        wp_safe_redirect(admin_url('options-general.php?page=jalin-mcp-gateway&tab=connections&connections_cleared=' . $deleted . '#fg-connections'));
        exit;
    }
    ...
    public static function review(): void {
        ...
        wp_safe_redirect(admin_url('options-general.php?page=jalin-mcp-gateway&tab=changes'));
        exit;
    }
    public static function page(): void {
        ...
        $base = admin_url('options-general.php?page=jalin-mcp-gateway');
        ?>
    ...
    private static function connect(array $settings): void {
        ...
        <section class="fg-card fg-connect-card" aria-labelledby="fg-connect-title">
            ...
            <p class="fg-current-mode"><strong>Change Mode:</strong> <?php echo esc_html(self::change_mode_label($settings)); ?> <a href="<?php echo esc_url(admin_url('options-general.php?page=jalin-mcp-gateway&tab=access#fg-change-mode')); ?>">Change Mode Settings</a></p>
            ...
            <ol class="fg-steps">
                <li><div class="fg-step-number" aria-hidden="true">1</div><div><h3>Your MCP Connection</h3>
                    ...
                    <?php endif; ?>
                    <p class="description">Order and financial data access, and change requests, stay as configured in <a href="<?php echo esc_url(admin_url('options-general.php?page=jalin-mcp-gateway&tab=access')); ?>">Access Controls</a>. Enable each explicitly when needed.</p>
                </div></li>
                <li><div class="fg-step-number" aria-hidden="true">2</div><div><h3>Copy Your Connector URL</h3><label class="screen-reader-text" for="fg-endpoint">MCP Server URL</label>
                    <div class="fg-copy-row"><input id="fg-endpoint" class="large-text code" type="url" readonly value="<?php echo esc_attr(rest_url('jalin-mcp/v1/mcp')); ?>" spellcheck="false"><button type="button" class="button fg-copy-button" data-copy-target="fg-endpoint" hidden>Copy URL</button></div><span id="fg-copy-status" class="fg-inline-status" role="status" aria-live="polite"></span>
                </div></li>
    ...
    private static function connections(): void {
        ...
        <section id="fg-connections" class="fg-card">
            <div class="fg-section-heading"><h2>OAuth Connections</h2><a class="button" href="<?php echo esc_url(admin_url('options-general.php?page=jalin-mcp-gateway&tab=connections#fg-connections')); ?>">Refresh Connections</a></div>
            ...
            <?php elseif (!$rows) : ?><p class="fg-empty">No OAuth connections are recorded. <a href="<?php echo esc_url(admin_url('options-general.php?page=jalin-mcp-gateway&tab=connect')); ?>">Open Connection Setup</a> to connect ChatGPT or Claude, then refresh this list.</p>
            <?php else : ?>
