...
final class FG_Finance_Admin {
    ...
    public static function boot(): void {
        ...
        add_action('admin_enqueue_scripts', static function ($hook) {
            if ($hook !== 'settings_page_jalin-mcp-gateway' || ($_GET['tab'] ?? '') !== 'finance') { return; }
            wp_enqueue_style('jalin-mcp-finance', plugins_url('assets/finance.css', FG_FILE), ['jalin-mcp'], FG_VERSION);
            wp_enqueue_script('jalin-mcp-finance', plugins_url('assets/finance.js', FG_FILE), [], FG_VERSION, true);
            wp_localize_script('jalin-mcp-finance', 'fgFinance', ['ajaxUrl' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('fg_finance_preview')]);
        });
    }

    private static function url(): string { return admin_url('options-general.php?page=jalin-mcp-gateway&tab=finance'); }
    ...
    public static function render(array $settings): void {
        ...
        <section class="fg-card fg-finance-intro">
            ...
            <p class="description">Reports require financial data access in <a href="<?php echo esc_url(admin_url('options-general.php?page=jalin-mcp-gateway&tab=access')); ?>">Access Controls</a> and your account's WooCommerce permissions.</p>
        </section>
        <?php
        $permission = self::permission();
        ...
        <section class="fg-card">
            ...
            <?php if (empty($settings['sensitive'])) : ?><p class="fg-finance-notice">Order tests are unavailable until you enable financial data access in <a href="<?php echo esc_url(admin_url('options-general.php?page=jalin-mcp-gateway&tab=access')); ?>">Access Controls</a>. You can still configure mappings.</p><?php endif; ?>
